...
import {
  ...
  XCircle, FileText, Users, ClipboardList, AlertCircle,
} from "lucide-react";
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 scrollbar-hide">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea required value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 outline-none h-24" placeholder="Describe the internship roles..." />
          </div>
          <div>
    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-all overflow-hidden mb-4">
      <div className="p-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-gray-100">
          ...
          <button onClick={() => setExpanded(!expanded)} className="text-blue-600 text-sm font-bold flex items-center gap-1 hover:underline">
              ...
              <div className="flex flex-col md:flex-row justify-between gap-4">
                ...
                <div className="flex flex-col items-end gap-3 self-center sm:self-start">
                  ...
                  {(app.status === "Pending" || app.status === "applied") ? (
                    <button onClick={() => setSelectedApplication(app)} className="flex items-center gap-1 px-3 py-1.5 bg-blue-600 text-white text-xs font-bold rounded-lg hover:bg-blue-700 shadow-sm transition-colors">
                ...
                <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-100">
                  ...
                  {app.additionalDocument && (
                    <div className="flex items-center gap-3 p-3 bg-white border border-blue-100 rounded-lg">
                      ...
                      <a href={app.additionalDocument} download={app.documentName || "CV.pdf"} className="px-3 py-1 bg-blue-50 text-blue-600 text-[10px] font-black uppercase rounded hover:bg-blue-100 transition-colors">Download</a>
                    </div>
                  )}
                </div>
              )}
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
        ...
        <div className="bg-blue-50 text-blue-600 px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest border border-blue-100">University Verified</div>
      </div>
                  ...
                  return (
                    <button type="button" onClick={() => openInternDetail(intern)} key={intern.id} className="text-left bg-white p-6 rounded-2xl border border-gray-100 hover:shadow-xl transition-all border-l-4 border-l-blue-600 group relative">
                      ...
                      <div className="mt-6 pt-4 border-t border-gray-50 flex justify-between items-center">
                        <div className="flex items-center gap-2 text-xs font-bold text-gray-500"><Mail className="w-3.5 h-3.5 text-blue-600" /><span className="truncate max-w-[120px]">{intern.studentFull?.email || "Academic Account"}</span></div>
        ...
        <div className="fixed inset-0 bg-black/60 z-[160] p-4 overflow-y-auto">
          <div className="max-w-5xl mx-auto my-8 bg-white rounded-xl shadow-xl border border-gray-200 p-4 sm:p-6">
            <div className="flex justify-between items-start mb-4">
              ...
              <button type="button" onClick={() => { setSelectedIntern(null); setSelectedRecord(null); setOpenEvalMonth(null); }} className="text-sm font-semibold text-gray-500 hover:text-gray-700 px-3 py-1.5 border border-gray-200 rounded-lg">Close</button>
            </div>
            ...
            {internDetailTab === "monthly" && (
              <div>
                {openEvalMonth === null ? (
                        ...
                        <div key={month} className="border border-gray-200 rounded-xl p-6 space-y-4 bg-gray-50/40">
                          ...
                          {rec?.evaluationData && (
                            <div className="text-sm text-gray-600 space-y-1">
                              ...
                              {rec.advisorComment && <p className="text-blue-700 text-xs mt-1">Advisor: {rec.advisorComment}</p>}
                            </div>
                          )}
                          <button type="button" onClick={() => setOpenEvalMonth(month)}
                            className={`w-full py-2 rounded-lg text-sm font-bold transition-colors ${canEdit ? "bg-blue-600 hover:bg-blue-700 text-white" : "bg-gray-100 hover:bg-gray-200 text-gray-700"}`}>
                  ...
                  <div>
                    <button type="button" onClick={() => setOpenEvalMonth(null)} className="mb-4 text-sm text-blue-600 hover:underline font-semibold">← Back to evaluations</button>
                    {(() => {
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-transparent">
                <div className="bg-blue-600 text-white h-8 w-8 rounded-full flex items-center justify-center font-bold text-xs">{cName.charAt(0)}</div>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <div className="flex flex-wrap gap-4 text-sm opacity-90 mt-2">
                <div className="flex items-center gap-2"><span className="font-bold">Email:</span><span>{email}</span></div>
                <div className="flex items-center gap-2 border-l border-white/20 pl-4"><span className="font-bold">Representative:</span><span>{repName}</span></div>
              </div>
            </div>
